...
$rowsCount = 0;
if ($rowsCountStmt->execute()) {
    $rowsCountRes = $rowsCountStmt->get_result();
    $row = $rowsCountRes ? $rowsCountRes->fetch_assoc() : ["rowsCount" => 0];
    $rowsCount = (int)$row["rowsCount"];
    if ($rowsCountRes) $rowsCountRes->free();
}
if ($rowsCountStmt) $rowsCountStmt->close();
...
if ($limitedContentStmt && $limitedContentStmt->execute()) {
    $limitedContentRes = $limitedContentStmt->get_result();
    if ($limitedContentRes) {
        // $fields = $limitedContentRes->fetch_fields();
        // foreach ($fields as $field) {
        //     $colsNames[] = $field->name;
        // }
        $rows = $limitedContentRes->fetch_all(MYSQLI_ASSOC);
        $limitedContentRes->free();
    }
}

if ($limitedContentStmt) $limitedContentStmt->close();
